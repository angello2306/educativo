...
class Admin::SectionsController < Admin::BackendController
  ...
  def index
    # Use for search
    @sections = Section.search params[:search], 10, params[:page]
  ...
  def update
    respond_to do |format|
      if @section.update(section_params)
        format.html { redirect_to edit_admin_section_path(@section), notice: 'Section was successfully updated.' }
  ...
  private
    ...
    def section_params
      params.require(:section).permit(:name, :grade_id)
    end
end
